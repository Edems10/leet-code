...
def main():
    ...
    for sort_kind in numpy_sorts:
        exec_time = timeit(lambda: narray_numpy_sorting(np_numbers_array, kind=sort_kind), number=NUMBER_OF_TESTS)
        execution_times.append((f"numpy {sort_kind}", exec_time))

    # Existing sorting functions
    execution_times.append(("wrapperNumpyQuickSort", timeit(lambda: wrapperNumpyQuickSort(qs), number=NUMBER_OF_TESTS)))
    execution_times.append(("wrapperMyQuickSort", timeit(lambda: wrapperMyQuickSort(qs), number=NUMBER_OF_TESTS)))
    execution_times.append(("default_sorting_algorithm", timeit(lambda: default_sorting_algorithm(numbers_array), number=NUMBER_OF_TESTS)))
    ...
    for name, exec_time in execution_times:
        ...
        print(f"{name:<30}{exec_time:.6f}{' ' * 5}{relative_speed:.2f}x faster than slowest")
