...
def main():
    ...
    print(f"Reversing with {brut_force.__class__.__name__} took: {brut_force.get_time()-init_time} ")
    
    two_point = TwoPoint()
    init_time =two_point.get_time()
    reversed_string = two_point.reverse_string(initial_string)
    print(f"Reversed string is: {reversed_string}")
    print(f"Reversing with {two_point.__class__.__name__} took: {two_point.get_time()-init_time} ")
